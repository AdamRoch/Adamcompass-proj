...
export async function listWithCounts(): Promise<Array<{ id: string; name: string; count: number }>> {
  const handle = getDb();
  const tags = await handle.db.select().from(handle.schema.tag);
  const results: Array<{ id: string; name: string; count: number }> = [];
  for (const t of tags) {
    const rows = await handle.db
      .select({ c: sql<number>`count(*)` })
      .from(handle.schema.tagging)
      .where(eq(handle.schema.tagging.tag_id, t.id));
    results.push({ id: t.id, name: t.name, count: Number(rows[0]?.c ?? 0) });
  }
